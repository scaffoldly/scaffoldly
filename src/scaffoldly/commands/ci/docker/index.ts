...
export class DockerService {
  ...
  async build(
    ...
    console.log('!!! created stages', JSON.stringify(stages, null, 2));
    ...
    return { imageName, entrypoint: entrypoint };
  }
        ...
        if (src !== DEFAULT_SRC_ROOT) {
          files.forEach((file) => {
            ...
            if (from && f) {
              copy.push({
                from: `${mode}-${from}`,
                src: join(workdir, join(src, f)),
                dest: join(workdir, join(src, f)),
              });
              return;
            }
  ...
  //       .map((s) => {
  //         console.log(`!!! checking spec ${s.as}`);
  //         return (s.copy || []).map((c) => {
  //           console.log(`!!!    considering copy`, c);
  //           let from = s.as;
  //           if (c.bin) {
  //             from = c.from;
  //           }
